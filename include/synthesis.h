...
static const char *SynthTypeDisplayNames[NUM_SYNTH_TYPES] = {
    "subtractive drum synth", "subtractive", "physical model",
    "frequency modulation", "sampler"};
...
template <typename sample_t> struct Synthesizer {
  ...
  union uSynthesizer {
    SubtractiveDrumSynth<sample_t> drumSynth;
    ...
    uSynthesizer(const SubtractiveDrumSynth<sample_t> &s) : drumSynth(s) {}
    ...
    while (!eventQueue.empty()) {
      ...
      case SynthesizerEvent<sample_t>::SYNTHESIZER_CHANGE: {
        ...
        case SUBTRACTIVE_DRUM_SYNTH: {
          ...
          object.drumSynth = SubtractiveDrumSynth<sample_t>();
          break;
        }
        case SUBTRACTIVE: {
          type = SUBTRACTIVE;
